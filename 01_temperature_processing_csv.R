...
#    PrePostResults
#    FieldAuditResults
# PLUS a folder with the continuous temperature data in csv files.
#
...
#
#
#Expected format for csv files with logger ID as their filename:
#    Column A = DATE
#    Column B = TIME
#    Column C = TEMPERATURE RESULT
# Data begins on the second row

#Set the data file path and file that you want to process
src_file <- '//deqlab1/WQM/OdfwOPlanCCohoTemp/2017ODFW/0234_ODFWCnT2017_4r.xls'


# csv file path..needs to have files with logger ID in csv file name, can't be more than one file with logger ID ending in CSV
cfp <- '//deqlab1/WQM/OdfwOPlanCCohoTemp/2017ODFW/DeployData/2017CSV/'
cf <- list.files(path = cfp, pattern = '\\.csv')

SubID <- '0234' # Enter the submission ID from VolWQDB
...
#logchar$log[!which(logchar$log %in% cf)]
...
for (i in seq_along(logchar$log)) {
  ...
  tmp_data <- read.csv(file = paste0(cfp,cf[pmatch(logchar[i,1], cf)]), skip = 1)
  tmp_data <- tmp_data[,1:3]
  names(tmp_data) <- c('DATE', 'TIME', 'TEMP_r')
  tmp_data$TEMP_DQL <- as.character(NA)
  tmp_data$charid <- as.character(logchar$char[1])
  ...
  tmp_data$date_char <- as.character(tmp_data$DATE)
  tmp_data$time_char <- as.character(tmp_data$TIME)
  ###  Fix time zone gaps S.Hanson 4/12/2019
  # get first date time
  
  #ttz1 <- as.POSIXct(strptime(paste(tmp_data$date_char[1], tmp_data$time_char[1]), format = "%Y-%m-%d %H:%M:%S", tz = 'America/Los_Angeles'))
  ttz1 <- as.POSIXct(strptime(paste(tmp_data$date_char[1], tmp_data$time_char[1]), format = "%m/%d/%y %H:%M:%S", tz = 'America/Los_Angeles'))
  #calculate UTC hour correcton
  hrUTCoffset <- ifelse(dst(ttz1) == TRUE, 7, 8)
  tmp_data$datetime <- paste(tmp_data$date_char, tmp_data$time_char)
  #tmp_data$DATETIME <- as.POSIXct(strptime(tmp_data$datetime, format = "%Y-%m-%d %H:%M:%S", tz = 'UTC')) + dhours(hrUTCoffset) # create datetimes in UTC
  tmp_data$DATETIME <- as.POSIXct(strptime(tmp_data$datetime, format = "%m/%d/%y %H:%M:%S", tz = 'UTC')) + dhours(hrUTCoffset) # create datetimes in UTC
  #convert to Pacific Time
  tmp_data$DATETIME <- with_tz(tmp_data$DATETIME, tzone = 'America/Los_Angeles')
  
  
  ifelse(length(unique(dst(tmp_data$DATETIME))) == 1, print("No Time Change"), print("Time Change Correction Needed"))
  # Deleted old script see \\deqlab1\WQM\Volunteer Monitoring\datamanagement\R\RcodeIssues.docx
  ...
  dr_obs <- plyr::rename(dr_obs, c('DATETIME' = "OBS_DATETIME",
                             'r' = 'LOGGED_RESULT'))
  ...
  tmp_data$date <- as.Date(tmp_data$DATETIME, tz="America/Los_Angeles") # removed format= "%m/%d/%Y", 
  ...
  date <- seq(min(tmp_data$date, na.rm = T), max(tmp_data$date, na.rm = T), by=1)
