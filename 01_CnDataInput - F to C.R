...
#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
#USE WHEN DATA IS  IN FAHRENHEIT
#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
src_file <- '//deqlab1/Vol_Data/umpqua/2012/4rPUR_2012_ContinousAuditFormat2.3.xls'

SubID <- '0108' # Enter the submission ID from VolWQDB



#Set the output location where the shiny app can use it
save_dir <- 'S:/DataManagement/ContinuousDataRTool/Data in Waiting/data/'



temp_converter <- function(temp_F) {
  temp_c <- (temp_F - 32) * (5 / 9) 
  temp <- round_any(temp_c, accuracy=0.01, f=floor) 
  return(temp)
}
...
ppcheck <- ppcheck %>%
  mutate(UNITS = "deg C", 
         EXPECTED_RESULT = temp_converter(EXPECTED_RESULT),
...
audits <- audits %>%
  mutate(UNITS = "deg C", 
         AUDIT_RESULT = temp_converter(AUDIT_RESULT),
         LOGGER_RESULT = temp_converter(LOGGER_RESULT))
