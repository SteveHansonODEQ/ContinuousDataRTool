...
SubID <- '2005178' # Enter the submission ID from VolWQDB
...
save_path <- "//deqlab1/WQM/TMDL/aEastern Region/Deschutes/2020/20200507/ContinuousData/ROutputs/"  # make sure this ends with an /
...
for (i in 1:length(validdatafiles)) { 
  ...
  newstartdt <- deploydatetime-(30*86400)
  
  # figure out which work orders have data for this date frame using allaudits 
  # need vector of unique work orders where charid, loggerID match fileinfo and auditdatetime >= newstartdt
  audrng <- filter(allaudits, (PARAMETER == fileinfo$charid & lasar == fileinfo$lasar & AUDIT_DATETIME >= newstartdt )) ##  NOT ROBUST for different loggers at same locations
  ...
  files2load <- grep(fileinfo$charid, files2load, value = T) # then reduce again to just files with matching charid
  ...
  #ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ  HERE HERE
  ...
  if (length(Files2LoadDF$path) >0 ) { for (f in 1:length(Files2LoadDF$path))
      ...
      if(!'cmnt' %in% names(tmp_data)) tmp_data$cmnt<- NA # moved this from below to get equal columns when combining data.
      alldata <- tmp_data
      if (f < length(Files2LoadDF$path)) { # if the added file is not the oldest file keep alldata
        ...
        tmp_data <- alldata # ...the alldata DF shold be renamed tmp_data so the rest of the script works
      }
   }  
  }
  # End of the process to append the last 30 days of data to the exisitng dataset.
  ...
  ifelse (is.na(ds$cmnt[which(ds$date == as.Date(deploydatetime))]), 
          ds$cmnt[which(ds$date == as.Date(deploydatetime))] <- "Daily stats include results from previous data collection", 
          ds$cmnt[which(ds$date == as.Date(deploydatetime))] <- paste0(ds$cmnt[which(ds$date == as.Date(deploydatetime))], " Daily stats include results from previous data collection")) 
